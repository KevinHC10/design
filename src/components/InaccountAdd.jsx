...
const InaccountAdd = (props) => {
  ...
  // 댓글 가져오기
  useEffect(() => {
    getComments(Number(props.inaccount_id));
  }, [props.inaccount_id]);

  const getComments = async (inaccount_id) => {
    const res = await axios.get(`http://localhost:8083/api/comment/list?inaccount_id=${inaccount_id}`);
  ...
  const submit = async (values) => {
    ...
    const comment = {
      inaccount_id: props.inaccount_id,
      in_account: values.in_account,
      user_id: jwtUtils.getId(token)
    }
    // axios interceptor 사용 : 로그인한 사용자만 쓸 수 있다!
    const res = await api.post('/api/comment', in_account);
      ...
      <div className="comment-list">
        {comments.map((item) =>
          <div key={item.id}>
            ...
            <div className="comment-content">{item.in_account}</div>
            <hr/>
          </div>
        )}
      </div>
